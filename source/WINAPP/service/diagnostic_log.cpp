...
void DiagnosticLogInit() {
    std::lock_guard<std::mutex> lock(s_log_mutex);
    s_log_path = ExeDir() + "\\telemetry_service_diagnostic.log";
    if (s_log.is_open()) s_log.close();
    s_log.open(s_log_path, std::ios::out | std::ios::trunc);
    if (!s_log.is_open()) return;

    s_log << Timestamp() << " [INFO] [pid=" << GetCurrentProcessId()
          << " tid=" << GetCurrentThreadId() << "] "
          << "TelemetryService diagnostic log opened. This file is overwritten each run.\n";
    s_log << Timestamp() << " [INFO] [pid=" << GetCurrentProcessId()
          << " tid=" << GetCurrentThreadId() << "] "
          << "Log path: " << s_log_path << "\n";
...
void DiagnosticLogShutdown() {
    WriteLine("INFO", "TelemetryService diagnostic log closing.");
    std::lock_guard<std::mutex> lock(s_log_mutex);
    if (s_log.is_open()) s_log.close();
}

...
> Auto-generated by TelemetryApp v1.0.3 on 2026-07-14 09:20 UTC  
> Capability revision: `2026-07-14.1.0.3-power-dashboard`  
> Docs bundle: README/API updated for dashboard power metrics, source-qualified electrical telemetry, Prometheus power labels, fleet, firewall, hardware inventory, installer role detection, release-control validation, and dashboard scrollbar usability  
...
Power/electrical shared-memory metric IDs are also exposed through `/api/v1/metrics/catalog`, `/api/v1/history/<id>`, session logging, and the dashboard:

| ID | Name | Unit | Meaning |
|----|------|------|---------|
| 393 | `platform_power` | watts | Platform power derived from Windows battery discharge rate when available |
| 394 | `battery_rate` | watts | Battery charge/discharge rate reported by Windows power APIs |
| 395 | `battery_percent` | percent | Battery charge percentage |
| 396 | `ac_power_state` | state | AC line status from Windows power APIs |
